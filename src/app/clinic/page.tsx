...
export default function ClinicPage() {
  const { user, loading, isClinic, clinicId } = useAuth();
  const [clinicData, setClinicData] = useState<ClinicData | null>(null);

  useEffect(() => {
    if (loading || !user || !isClinic || !clinicId) return;
    ...
    getDoc(clinicRef).then(docSnap => {
        if (docSnap.exists()) {
            ...
            // Handle case where clinic user has no corresponding Firestore document
        }
    });
  }, [user, loading, isClinic, clinicId]);
